...
def add(name):
    ...
    firestore_db.collection(collectionName).add(
        {'name': name, 'date': now.split()[0], 'time': now.split()[1]})
...
def getScore(season):
    ...
    print(name)
    snapshots = [x.to_dict()
                 for x in list(firestore_db.collection(name).get())]
...
def getLastScore():
    ...
    collectionName = 'counter log s' + str(currentSeason)
    snapshots = [x.to_dict()
                 for x in list(firestore_db.collection(collectionName).get())]
    ...
    result['time'] = (dt.strptime(result['time'], "%H:%M:%S.%f") +
                      datetime.timedelta(hours=7)).strftime("%H:%M:%S")
    return result
